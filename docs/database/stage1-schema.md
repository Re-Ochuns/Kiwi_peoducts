...
Issue #10（S1-01）の受入・選果・在庫・ラベル・変更履歴の基盤を定義する。受入、選果、ラベルの更新RPCは後続Issue #12、#14、#17で実装する。
...
- `private.idempotency_records`はservice roleだけが操作でき、応答を24時間以上保持する。`private.delete_expired_idempotency_records()`を定期実行または更新RPC開始時に呼び出す。
