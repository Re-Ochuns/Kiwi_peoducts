...
Issue #10（S1-01）の受入・選果・在庫・ラベル・変更履歴の基盤を定義する。受入の更新RPCはIssue #12（[受入RPC運用](receiving-rpc.md)）、選果の更新RPCはIssue #14（[選果RPC運用](sorting-rpc.md)）、ラベルの状態管理RPCとPDF生成はIssue #17（[ラベルRPC運用](label-rpc.md)・[label-pdf Edge Function](../edge-functions/label-pdf.md)）で実装済み。
