...
2. 農園共通カレンダーを、そのclient_emailへ「予定の変更」権限で共有する。
   個人のprimaryカレンダーを指定せず、共有カレンダーのIDを確認する。
   出席者を登録しないため、ドメイン全体の代理権限は使用しない。
3. 秘密情報をリポジトリ外の保護されたenvファイルへ保存する。

| Edge Function環境変数 | 内容 |
|---|---|
| GOOGLE_CALENDAR_SERVICE_ACCOUNT | client_email/private_keyを含むサービスアカウントJSON（1行） |
| GOOGLE_CALENDAR_ID | 農園共通カレンダーID |
| APP_BASE_URL | HTTPSのアプリURL（例示URLを実環境のものへ置換） |
| CALENDAR_SYNC_TOKEN | 十分に長いランダムなサーバー間共有トークン |
| SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY | Supabaseが提供するサーバー用設定 |

設定ファイルをコミットしない。秘密情報をPR・Issue・ログへ貼らない。

```bash
supabase secrets set --env-file /secure/calendar-sync.env --project-ref <project-ref>
supabase functions deploy calendar-sync --project-ref <project-ref>
```

4. Supabase Vaultへ次の名前で値を登録する。
   calendar_sync_url = https://<project-ref>.supabase.co/functions/v1/calendar-sync
   calendar_sync_token = Edge FunctionのCALENDAR_SYNC_TOKENと同一の値。
   SQL Editorでvault.create_secretまたはvault.update_secretを使用し、実際の値を履歴共有しない。
...
7. 専用の検証カレンダーで正常送信、手動編集の上書き、中止、権限削除時の失敗と復旧を確認する。
