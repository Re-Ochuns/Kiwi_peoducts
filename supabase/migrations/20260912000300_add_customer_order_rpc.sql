  ...
  if function_name_value = 'customer_register' then
    ...
    if function_name_value = 'order_update' then
      ...
      if order_row.status='confirmed' then
        update public.ripening_lots l set
          status=case when l.status='confirmed' then 'draft' else l.status end,
        ...
        get diagnostics affected_plans_value = row_count;
      end if;
      update public.orders set customer_id=(data_value->>'customer_id')::uuid,
        ...
        shipping_destination_snapshot=data_value->'shipping_destination_snapshot',
      ...
      update public.ripening_lots l set status='draft',needs_review=true,
        version=l.version+1,updated_by=actor_id_value
      where l.id in(select a.ripening_lot_id from public.ripening_allocations a
        where a.order_id=order_row.id) and l.status in ('draft','confirmed');
      get diagnostics affected_plans_value = row_count;
      for allocation_row in select a.id,a.ripening_lot_id,a.allocated_weight_kg
        from public.ripening_allocations a where a.order_id=order_row.id order by a.id
      loop
        delete from public.ripening_allocations where id=allocation_row.id;
        perform private.release_order_reservations(
          allocation_row.ripening_lot_id,allocation_row.allocated_weight_kg,actor_id_value);
      end loop;
...
revoke all on function private.release_order_reservations(uuid,numeric,uuid) from public,anon,authenticated;
