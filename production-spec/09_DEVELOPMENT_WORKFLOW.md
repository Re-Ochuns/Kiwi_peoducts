...
- Local、Staging、Productionを分離し、実在する顧客情報をLocalとStagingへ投入しない。
...
```text
Backlog → Ready → In Progress → Pull Request → Staging Review → Done
```

- `Ready`へ移す前に受け入れ条件と依存Issueを確定する。
- 作業開始時に担当者を設定し、ブランチとPull RequestからIssueをリンクする。
- Staging確認前にIssueを完了しない。
...
- Staging確認後：当初仕様の未達は不具合、改善要求は新しいIssueとする。
